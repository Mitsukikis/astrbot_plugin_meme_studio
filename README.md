...
启动后浏览器会打开本地页面。终端会打印一个带 `token` 的访问地址，管理 API 需要这个 token 才能调用。

Linux 服务器或 Docker 环境中，推荐在插件目录直接启动：
...
```text
Meme Studio running at http://0.0.0.0:8765/?token=...
```

在浏览器访问时，将 `0.0.0.0` 换成服务器 IP 或域名，并保留完整 `token` 参数。这个 URL 等同于管理钥匙，请不要发到群聊、公开日志或截图里。更稳妥的做法是通过 SSH 隧道访问：

```bash
ssh -L 8765:127.0.0.1:8765 user@your-server
```

然后在服务器上用下面的命令启动 Studio，本机浏览器打开终端打印的本地地址：

```bash
python tools/meme_studio.py --host 127.0.0.1 --port 8765 --no-open
```

需要 Windows 双击入口时，可以在本地运行下面的命令自行构建：
...
1. 运行 `python tools/meme_studio.py`；服务器部署时按上面的 Linux 命令启动。
2. 点击“新增表情”。
3. 填写指令名，例如 `拍脑袋`。
4. 上传一张图片或一个 GIF。
5. 拖动头像框到合适位置。
...
- 机器人消息中的 Base64 图片使用严格解码，异常输入会被拒绝。
- Meme Studio 上传素材限制为 PNG、JPG、JPEG、GIF、WebP，单文件 20 MB，单次最多 32 个文件；data URL 的媒体类型必须和文件扩展名一致。
